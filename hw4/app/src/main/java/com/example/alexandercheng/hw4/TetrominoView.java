...
public class TetrominoView extends View {
    ...
    public TetrominoView(Context context, TGrid tg){
        super(context);
        setLayerType(View.LAYER_TYPE_SOFTWARE, null);
        tgrid = tg;
    }
    ...
    public void onDraw(Canvas canvas){
        ...
        cellHeight = getHeight()/(tgrid.getHeight());

        canvas.drawColor(Color.WHITE);
        lines.setColor(Color.BLACK);

        for (int i = 0; i < (tgrid.getHeight()); i++)
        {
            canvas.drawLine(0, i * getHeight()/(tgrid.getHeight()), getWidth(), i * getHeight()/(tgrid.getHeight()),
                    lines);
        ...
        for (int x = 0; x < tgrid.getWidth(); x++) {
            for (int y = 0; y < (tgrid.getHeight()); y++) {
